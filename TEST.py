...
def drawlabel(image, final_matches, gt_dists, dist_calc):

    # 1. Memory of where labels are already placed: [x1, y1, x2, y2]
    occupied_regions = []
    ...
    for i, (gt_idx, dt_idx) in enumerate(final_matches):
        # Get coordinates of the YOLO box (dt_idx)
        # Assuming you have a list or array 'det_boxes'
        x1, y1, x2, y2 = det_boxes[dt_idx] 
        
        # Format the text (Distance and IoU)
        label_text = f"GT: {gt_dists[i]:.2f}m \n YOLO: {dist_calc[i]:.2f}m"
        
        # Get dimensions of this specific piece of text
        (w, h), _ = cv2.getTextSize(label_text, font, scale, thickness)
        
        # Initial target: Top-center of the car
        curr_x = int((x1 + x2)/2 - w/2)
        curr_y = y1 - 10 

        # 2. Collision Search Logic
        # If the preferred spot is taken, we keep moving UP until it's clear
        while True:
            # Define the rectangle this label wants to occupy
            candidate_rect = [curr_x - padding, curr_y - h - padding, 
                              curr_x + w + padding, curr_y + padding]
            
            overlap = False
            for region in occupied_regions:
                # Box-to-Box collision check (AABB)
                if not (candidate_rect[2] < region[0] or 
                        candidate_rect[0] > region[2] or 
                        candidate_rect[3] < region[1] or 
                        candidate_rect[1] > region[3]):
                    overlap = True
                    break
            ...
            if not overlap:
                ...
                # Shift up by height + a small gap and check again
                curr_y -= (h + 12)

        # 3. Draw the label once a clear spot is found
        # Black background plate for visibility
        cv2.rectangle(image, (candidate_rect[0], candidate_rect[1]), 
                      (candidate_rect[2], candidate_rect[3]), (0, 0, 0), -1)
        
        # Cyan text for the data
        cv2.putText(image, label_text, (curr_x, curr_y), font, scale, 
                    (255, 255, 0), thickness, cv2.LINE_AA)
        
        # Add this spot to the 'no-go' zone for the next car
        occupied_regions.append(candidate_rect)
...
for img_path, label_path, calib_path in zip(img_paths, label_paths, calib_paths):  
    ...
    for gt_idx, dt_idx in zip(match_GT, match_DT):
        ...
        dist_gt_graph.append(dist_gt[gt_idx])
        dist_dt_graph.append(dist_dt[dt_idx])
        # x1, y1, x2, y2 = det_boxes[dt_idx]
        # x_center = (x1 + x2) // 2
        # y_center = (y1 + y2) // 2
        # label_text = f"{dist_dt[dt_idx]:.2f}m"

    drawlabel(canvas, list(zip(match_GT, match_DT)), dist_gt, dist_dt)
